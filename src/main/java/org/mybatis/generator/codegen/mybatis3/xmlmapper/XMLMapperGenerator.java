...
public class XMLMapperGenerator extends AbstractXmlGenerator {
    ...
    protected XmlElement getSqlMapElement() {
        ...
        addCreateElement(answer);
        addSelectColumnsElement(answer);
        addDeleteByIdElement(answer);
        addLoadAllElement(answer);
        addFindByIdElement(answer);
        addPageConditionsElement(answer);
        addFindBySelectiveElement(answer);
        addFindBySelectiveCountElement(answer);
        addExistsBySelectiveElement(answer);
        addUpdateSqlElement(answer);
    ...
    protected void addUpdateSqlElement(XmlElement parentElement) {
        if (introspectedTable.getRules().generateBaseResultMap()) {
            AbstractXmlElementGenerator elementGenerator = new UpdateSqlElementGenerator();
            initializeAndExecuteGenerator(elementGenerator, parentElement);
        }
    }
    protected void addExistsBySelectiveElement(XmlElement parentElement) {
        if (introspectedTable.getRules().generateBaseResultMap()) {
            AbstractXmlElementGenerator elementGenerator = new ExistsBySelectiveElementGenerator();
            initializeAndExecuteGenerator(elementGenerator, parentElement);
        }
    }
    protected void addFindBySelectiveCountElement(XmlElement parentElement) {
        if (introspectedTable.getRules().generateBaseResultMap()) {
            AbstractXmlElementGenerator elementGenerator = new FindBySelectiveCountElementGenerator();
            initializeAndExecuteGenerator(elementGenerator, parentElement);
        }
    }

    protected void addFindBySelectiveElement(XmlElement parentElement) {
        if (introspectedTable.getRules().generateBaseResultMap()) {
            AbstractXmlElementGenerator elementGenerator = new FindBySelectiveElementGenerator();
            initializeAndExecuteGenerator(elementGenerator, parentElement);
        }
    }
    protected void addPageConditionsElement(XmlElement parentElement) {
        if (introspectedTable.getRules().generateBaseResultMap()) {
            AbstractXmlElementGenerator elementGenerator = new PageConditionsElementGenerator();
            initializeAndExecuteGenerator(elementGenerator, parentElement);
        }
    }
    ...
    protected void addFindByIdElement(XmlElement parentElement) {
        if (introspectedTable.getRules().generateBaseResultMap()) {
            AbstractXmlElementGenerator elementGenerator = new FindByIdElementGenerator();
            initializeAndExecuteGenerator(elementGenerator, parentElement);
        }
    }
    protected void addLoadAllElement(XmlElement parentElement) {
        if (introspectedTable.getRules().generateBaseResultMap()) {
            AbstractXmlElementGenerator elementGenerator = new LoadAllElementGenerator();
            initializeAndExecuteGenerator(elementGenerator, parentElement);
        }
    }
    ...
    protected void addSelectColumnsElement(XmlElement parentElement) {
        if (introspectedTable.getRules().generateBaseResultMap()) {
            AbstractXmlElementGenerator elementGenerator = new SelectColumnsElementGenerator();
            initializeAndExecuteGenerator(elementGenerator, parentElement);
        }
    }
    ...
    protected void addCreateElement(XmlElement parentElement) {
        if (introspectedTable.getRules().generateBaseResultMap()) {
            AbstractXmlElementGenerator elementGenerator = new CreateElementGenerator(false);
            initializeAndExecuteGenerator(elementGenerator, parentElement);
        }
    }
